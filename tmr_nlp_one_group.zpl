...
#|"avg"|19.000, 14.595, 13.789, 99.704, 2012.708, 2.102, 0.014|
|"a1"|15.899, 14.676, 13.243, 94.669, 1864.875,  2.077,  0.201|
|"a2"|25.346, 14.517, 14.960, 110.291, 2311.872, 2.143, -0.347|
;
...
var intake_scale[Cows];
subto fix_scale: intake_scale["a1"] == 1;
var gsr[Cows] real;

var intake_kg_conc real;
var ffvs real;
var fufl real;
var fkgs real;
var def real;
...
subto need: forall <c,n> in Cows * Nutrients do
  sum <f> in Feeds: feed_data[f,n] / cow_data[c,n] * intake_scale[c] * intake_kg[f] == 1 + surplus[c,n] - deficit[c,n];
...
subto rnb: forall <c> in Cows do
  0 <= sum <f> in Feeds: feed_data[f, "RNB"] * intake_scale[c] * intake_kg[f] <= 50;

subto conc_max: forall <c> in Cows do
  sum <f> in Feeds:
    if (feed_data[f, "GSR"] > 0) then
      (1 - cc_intake_max) / cc_intake_max * intake_kg[f] 
    else
      - intake_kg[f] end <= 0;
...
subto conc_kgs: sum <k> in Concentrates: intake_kg[k] - intake_kg_conc == 0;
subto gsr: forall <c> in Cows do
  if (cow_data[c, "BWC"] < 0) then
    -0.43 + 1.82 * gsr_zero[c] + 0.035 * intake_scale[c] * intake_kg_conc - 0.00053 * cow_data[c, "PLPOT"] * intake_scale[c] * intake_kg_conc
  else
    gsr_zero[c]
  end == gsr[c];
